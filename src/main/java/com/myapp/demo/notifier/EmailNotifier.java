...
public class EmailNotifier implements Notifier {
    ...
    @Autowired
    private com.myapp.demo.repository.StaffRepository staffRepository;
    ...
    public void notify(InventoryStock item) {
        ...
        try {
            java.util.List<com.myapp.demo.entity.Staff> admins = staffRepository
                    .findByRights(com.myapp.demo.entity.UserRights.ADMIN);

            if (admins.isEmpty()) {
                logger.warn("No admins found to send low stock alert for product: {}", item.getProductName());
                return;
            }

            for (com.myapp.demo.entity.Staff admin : admins) {
                ...
                logger.info("Email notification sent to admin {} for product: {}", admin.getEmail(),
                        item.getProductName());
            }
    ...
    private String buildHtmlEmailContent(InventoryStock item) {
        ...
        return "<!DOCTYPE html>\n" +
                ...
                "                    <div class=\"info-value\" style=\"color: #1f2937;\">" + item.getProductName()
                + "</div>\n" +
                "                </div>\n" +
                "                \n" +
                "                <div class=\"info-box\">\n" +
                "                    <div class=\"info-label\">🆔 Product ID</div>\n" +
                "                    <div class=\"info-value\" style=\"color: #6366f1;\">#" + item.getProductId()
                + "</div>\n" +
                "                </div>\n" +
                "                \n" +
                "                <div class=\"info-box\">\n" +
                ...
                "                    <div class=\"info-value\" style=\"color: #059669;\">₹" + item.getPricePerUnit()
                + "</div>\n" +
                "                </div>\n" +
                "                \n" +
                "                <div class=\"info-box\">\n" +
                "                    <div class=\"info-label\">💵 Total Value</div>\n" +
                "                    <div class=\"info-value\" style=\"color: #d97706;\">₹" + item.getTotalPrice()
                + "</div>\n" +
                "                </div>\n" +
                "            </div>\n" +
                "            \n" +
